...
// encryptfileCmd represents the encryptfile command
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		crypter.EncryptFile(absFilePath)
	},
}

func init() {
	rootCmd.AddCommand(encryptfileCmd)
	encryptfileCmd.Flags().StringP("filepath", "f", "", "Absolute path of the file to be encrypted")

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// encryptfileCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// encryptfileCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
